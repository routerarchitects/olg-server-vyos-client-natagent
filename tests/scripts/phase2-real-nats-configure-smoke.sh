...
# Phase 2 real-NATS configure smoke test for vyos-nats-agent.
#
# Proves:
...
# - sends a configure command using nats-agent-core SubmitConfigure
# - observes the Phase 2 placeholder configure result on result.vyos
...
#
# Usage:
#   chmod +x tests/scripts/phase2-real-nats-configure-smoke.sh
#   tests/scripts/phase2-real-nats-configure-smoke.sh
#
# Optional:
#   NATS_PORT=4223 tests/scripts/phase2-real-nats-configure-smoke.sh
#   NATS_KILL_WITH_SUDO=always tests/scripts/phase2-real-nats-configure-smoke.sh
#   PRINT_LOGS_ON_PASS=true KEEP_SMOKE_ARTIFACTS=true tests/scripts/phase2-real-nats-configure-smoke.sh
...
CONTROLLER_DIR="${ROOT_DIR}/.tmp/phase2-configure-smoke-controller"
...
ensure_nats_port_available() {
  ...
  if [[ -z "${pid}" ]]; then
    ...
    fail "port ${port} is already open, but owning process could not be identified or stopped. Try: NATS_PORT=4223 ./tests/scripts/phase2-real-nats-configure-smoke.sh or stop existing nats-server manually."
  fi
...
rpcID := fmt.Sprintf("phase2-smoke-configure-%d", time.Now().UnixNano())
uuid := fmt.Sprintf("cfg-phase2-smoke-%d", time.Now().UnixNano())
_, err = client.SubmitConfigure(ctx, agentcore.ConfigureCommand{
		Version:   wireVersion,
		RPCID:     rpcID,
		Target:    target,
		UUID:      uuid,
		Payload:   json.RawMessage(`{"hostname":"phase2-smoke-router"}`),
		Timestamp: time.Now().UTC(),
	})
	if err != nil {
		fatalf("submit configure command: %v", err)
	}
	fmt.Printf("[CONTROLLER] submitted configure rpc_id=%s uuid=%s\n", rpcID, uuid)

	result := waitForResult(ctx, resultCh, rpcID, uuid, "configure")
	if result.Result != "failure" || result.ErrorCode != "not_implemented" {
		fatalf("unexpected phase2 configure result: result=%q error_code=%q message=%q", result.Result, result.ErrorCode, result.Message)
	}
...
func waitForResult(ctx context.Context, ch <-chan agentcore.ResultEnvelope, rpcID string, uuid string, commandType string) agentcore.ResultEnvelope {
	for {
		select {
		case <-ctx.Done():
			fatalf("timed out waiting for result rpc_id=%s: %v", rpcID, ctx.Err())
		case msg := <-ch:
			if msg.RPCID == rpcID && msg.UUID == uuid && msg.CommandType == commandType {
				return msg
			}
		}
	}
...
echo "[PASS] Phase 2 real-NATS configure smoke test passed"
